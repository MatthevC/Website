...
if (!tocScrollListenerReady) {
  ...
  window.addEventListener('scrollend', () => {
    if (tocScrollLock) releaseTocScrollLock();
  }, { passive: true });
}
  ...
  if (target) setTimeout(() => {
    if (targetId === 'moderator-benefits-section') {
      ...
      if (block) {
        ...
        setTimeout(() => block.classList.remove('sidebar-nav-highlight-group'), 6000);
      }
    } else {
...
function setupDixperPage() {
  ...
  cards.forEach(card => {
    ...
    bindPlayButton(card);
    // Podgląd jest ładowany przez osadzony player Twitch, tak samo jak w Polecanych streamerach.
  });
}
...
function setupGlobalPageNavigation() {
  ...
  const isDiscordJoinPage = currentPath === "discord/join";
  ...
  try {
    ...
    const events = (data || []).map(event => {
      ...
      return {
        ...
        mainImageFit: meta.mainImageFit || event.main_image_fit || "contain",
